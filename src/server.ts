...
> extends Stream<Server.Event<T, R>> {
  options: Server.Options<T, R> & { encode: Function; decode: Function };
  server?: Bun.Server;
   ...
   */
  constructor(options?: Server.Options<T, R>) {
    super();
    const { encode = JSON.stringify, decode = JSON.parse } = options ?? {};
    this.options = { ...options, encode, decode };
  }
